...
// Utility to generate consistent avatar background color from string
...
// Utility to create avatar initials and style
function stringAvatar(name) {
  const initials = name ? name.split(' ').map((n) => n[0]).join('').toUpperCase() : 'BD';
  return {
    sx: { bgcolor: stringToColor(name || 'Default Billing Desk User'), fontWeight: 'bold', fontSize: 20 },
    children: initials,
  };
}
...
export default function BillingDeskProfileMenu({ user, onLogout, onProfilePicChange, loading }) {
  const theme = useTheme();
  const [anchorEl, setAnchorEl] = useState(null);
  const open = Boolean(anchorEl);

  const [localPic, setLocalPic] = useState(user?.profilePic || null);
  ...
  useEffect(() => {
    // Update localPic if user.profilePic changes externally
    if (user?.profilePic && user.profilePic !== localPic && !localPic?.startsWith('blob:')) {
      setLocalPic(user.profilePic);
    }
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        if (onProfilePicChange) {
          onProfilePicChange(updatedUser.image);
        }
  ...
  const handleNameUpdate = async () => {
    setIsUpdating(true);
    try {
      const res = await fetch(`http://localhost:2002/api/users/${user.userId}`, {
        method: 'PATCH',
  ...
  return (
    <>
      ...
      >
        {isEditingName ? renderNameUpdateForm() : isEditingPassword ? renderPasswordUpdateForm() : (
          <Box sx={{ p: 2 }}>
